package com.eminyidle.tour.adapter.in.web;

import com.eminyidle.tour.application.dto.*;
import com.eminyidle.tour.application.dto.req.*;
import com.eminyidle.tour.application.dto.res.SearchTourDetailRes;
import com.eminyidle.tour.application.dto.res.SearchTourRes;
...
public class TourController {
    ...
    public ResponseEntity<SearchTourDetailRes> searchTourDetail(@PathVariable String tourId, @RequestHeader(HEADER_USER_ID) String userId) {
        ...
        return ResponseEntity.ok(
                SearchTourDetailRes.builder()
                        ...
                        .cityList(tourDetail.getCityList().stream().map(city ->
                                TourCity.builder().cityName(city.getCityName()).countryCode(city.getCountryCode()).build()
                        ).toList())
    ...
    @GetMapping()
    public ResponseEntity<List<SearchTourRes>> searchTourList(@RequestHeader(HEADER_USER_ID) String userId) {
        log.debug("searchTourList: " + userId);
        return ResponseEntity.ok(tourService.searchTourList(userId).stream().map(tour ->
                SearchTourRes.builder()
                        .tourId(tour.getTourId())
                        .tourTitle(tour.getTourTitle())
                        .startDate(tour.getStartDate())
                        .endDate(tour.getEndDate())
                        .cityList(tour.getCityList().stream().map(city ->
                                TourCity.builder()
