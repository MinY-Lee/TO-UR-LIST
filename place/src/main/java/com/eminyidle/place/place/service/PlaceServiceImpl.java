...
public class PlaceServiceImpl implements PlaceService {
	...
	@Override
	public List<SearchPlaceListRes> searchPlaceList(String keyword, Float longitude,
		Float latitude) {
		...
		String locationBias =
			"{\"circle\": {" + "\"center\": { \"latitude\":" + latitude + ", \"longitude\":"
				+ longitude + "}, \"radius\":" + 50000 + "}}";
			...
			if (responseEntity != null) {
				...
				return responseEntity.getBody().getPlaces().stream().map(place -> {
					SearchPlaceListRes searchPlaceRes = SearchPlaceListRes.builder()
						...
						.placePhotoList(place.getPhotos() == null ? new ArrayList<>()
							: place.getPhotos().stream().map(photo -> photo.getName()).toList())
						.build();
		...
		if (responseEntity.getStatusCode().is2xxSuccessful()) {
			...
			if (responseEntity != null) {
				placeInfo = PlaceInfo.builder()
					...
					.placeOpenNow(responseEntity.getBody().getCurrentOpeningHours() == null ? null
						: responseEntity.getBody().getCurrentOpeningHours().getOpenNow())
					.placeWeekdayDescriptions(
						responseEntity.getBody().getCurrentOpeningHours() == null ? null
							: responseEntity.getBody().getCurrentOpeningHours()
								.getWeekdayDescriptions())
					.placeAcceptCreditCards(
						responseEntity.getBody().getPaymentOptions() == null ? null
							: responseEntity.getBody().getPaymentOptions().getAcceptsCreditCards())
					.placeAcceptCashOnly(responseEntity.getBody().getPaymentOptions() == null ? null
						: responseEntity.getBody().getPaymentOptions().getAcceptsCashOnly())
					.placePhotoList(responseEntity.getBody().getPhotos() == null ? new ArrayList<>()
						: responseEntity.getBody().getPhotos().stream()
							.map(photo -> photo.getName()).toList())
					.build();
			}
		} else {
	...
	public TourPlaceMessageInfo addPlace(LinkedHashMap<String, Object> body, String tourId,
		...
		if (checkPlaceDuplication(tourId, tourDay, placeId) == false) {
			...
			try {
				...
				placeRepository.createDoRelationship((String) body.get("tourId"),
					UUID.randomUUID().toString(), (String) body.get("placeId"),
					(String) body.get("placeName"), (Integer) body.get("tourDay"),
					tourPlace.getTourPlaceId());
				isSuccess = true;
	...
	// 장소 삭제
	@Override
	public TourPlaceMessageInfo deletePlace(LinkedHashMap<String, Object> body, String tourId,
		Map<String, Object> headers) {
		// 받아 온 body = requestbody
		Object responseBody = body;
		boolean isSuccess = false;
	...
	// 장소 날짜 수정
	@Override
	public TourPlaceMessageInfo updatePlace(LinkedHashMap<String, Object> body, String tourId,
		Map<String, Object> headers) {
		// 받아 온 body = requestbody
		Object responseBody = body;
		boolean isSuccess = false;
		...
		try {
			...
			if (checkPlaceDuplication(tourId, newTourDay, placeId)) {
				...
				log.info("새로운 장소 생성");
				tourPlaceId = placeRepository.updateTourDay(tourId, placeId, oldTourDay,
					newTourDay);
				isSuccess = true;
			}
	...
	public Boolean checkPlaceDuplication(String tourId, Integer tourDay, String placeId) {
		try {
			...
			String ans = doRelationRepository.findPlaceByTourIdAndPlaceId(tourId, tourDay, placeId)
...
//            Do ans = doRelationRepository.findPlaceByTourIdAndPlaceId(tourId, placeId).orElseThrow(NoSuchElementException::new);
		...
		try {
			return tour.getPlaceList().stream().map(place -> {
				TourPlaceInfo tourPlaceInfo = TourPlaceInfo.builder()
					...
					.activityList(place.getTourPlace().getActivityList() == null ? new ArrayList<>()
						: place.getTourPlace().getActivityList().stream()
							.map(activity -> activity.getActivity()).toList())
					.build();
	...
	public List<String> searchTourPlaceActivity(String tourId, Integer tourDay, String placeId) {
		...
		List<String> enrollActivity = placeRepository.findActivityByTourIdAndTourDayAndTourPlaceId(
			tourId, tourDay, placeId);
	...
	// 투어 업데이트(기간 변경)
	@Override
	public void updateTour(String tourId, String tourName, LocalDateTime start, LocalDateTime end,
		Integer period) {
		tourRepository.updateTour(tourId, tourName, start, end, period);
	}

	@Override
	public String requestPhoto(String placeId, String photoResource) {
		WebClient webClient = WebClient.builder()
			.baseUrl("https://places.googleapis.com")
			.build();

		String uri = "/v1/places/" + placeId + "/photos/" + photoResource + "/media";

		PhotoInfo result = webClient.get().uri(uriBuilder -> uriBuilder
				.path(uri)
				.queryParam("key", googleMapKey)
				.queryParam("maxWidthPx", 400)
				.build())
			.retrieve()
			.bodyToMono(PhotoInfo.class)
			.block();

		return result.getPhotoUri();
	}
}
