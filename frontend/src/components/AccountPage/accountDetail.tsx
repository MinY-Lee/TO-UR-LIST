...
export default function AccountDetail(props: PropType) {
  ...
  const DataPerDay = (data: AccountInfo[]) => {
    ...
    groupedData.sort(
      (a, b) =>
        new Date(a.payDatetime).getTime() - new Date(b.payDatetime).getTime()
    );
                ...
                <div key={index} className="px-5 mb-5">
                  ...
                  <div>
                    {data.data.map((item, index) => (
                      <div
                        onClick={() => {
                          navigate(`/tour/${tourId}/account/${item.payId}`, {
                            state: { item: item },
                          });
                        }}
                        key={index}
