...
export default function searchBar(props: ChildProps) {
    ...
    return (
        <div className="flex flex-col items-center color-bg-blue-4 rounded-lg p-2">
            ...
            <div className="m-2 h-[20vh] overflow-scroll w-[90%]">
                
                {hasSearched && resultList.length === 0 ? (
                    <div className="text-lg text-center text-gray-500">검색 결과가 없습니다.</div>
                ) : (
                    resultList.map((res, index) => (
                        <div onClick={isOurTeamMember(res.userNickname) ? () => handleAddMember(res) : undefined} key={index} className="flex justify-between m-2 items-center">
                            <div className="text-lg">{res.userNickname} ({res.userName})</div>  
                            <div hidden={isOurTeamMember(res.userNickname)} className='color-text-blue-1'>멤버</div>
                        </div>
                    ))
                )}
            </div>
