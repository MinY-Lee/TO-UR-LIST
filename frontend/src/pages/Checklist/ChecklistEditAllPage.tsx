...
export default function ChecklistEditAllPage() {
    ...
    const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (event.key === 'Enter') {
            const existingItem = filteredChecklist.find((item) => item.item === itemInput);
            if (existingItem) {
    ...
    const handleDelete = () => {
        // 데이터 삭제 api
        const deletedChecklist = filteredChecklist.filter((item) => item != deleteItem)
        setFilteredChecklist(deletedChecklist);
        closeModal();
    }
